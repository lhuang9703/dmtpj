...
class VideoServer(object):
    ...
    def __init__(self, port):
        """
        VideoServer类的初始化：绑定并监听端口，等待客户端连接
        :param port: [int] 视频服务器监听的端口号
        """
        self.address1 = ("", port)
        self.sock1 = socket(AF_INET, SOCK_STREAM)
        self.sock1.bind(self.address1)
        self.sock1.listen(10)
        print("video server start at:{},waiting for connection...".format(port))
        self.run_server()

    def __del__(self):
        """
        VideoServer类的析构，关闭套接字
        """
    ...
    def tcp_link(self, con, addr):
        """
        处理客户端发来的视频数据，将视频数据转发给所有客户端
        :param con: [class] 发来视频数据的客户端连接类
        :param addr: [string] 发来视频数据的客户端地址
        :return:
        """
        # 在con2video字典中注册该用户
        con2video[con] = 1
